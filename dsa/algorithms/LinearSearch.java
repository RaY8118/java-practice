...
public class LinearSearch {
  ...
  /**
   * Performs a linear search using Java Stream API to find a specified item in an integer array.
   * This method returns an {@code OptionalInt} which will contain the item if found,
   * or be empty if the item is not present.
   *
   * @param arr The array to be searched.
   * @param item The item to search for.
   * @return An {@code OptionalInt} containing the item if found, otherwise an empty {@code OptionalInt}.
   *
   * Time Complexity: O(n), where n is the number of elements in the array.
   * Space Complexity: O(n) in some stream implementations due to intermediate operations,
   *   though effectively O(1) for simple filtering and finding the first element.
   */
  ...
  public static void main(String[] args) {
    ...
    int item1 = 3;
    System.out.println("Using linearSearch: Is " + item1 + " found? " + (linearSearch(arr, item1) != -1 ? "Yes" : "No") + " (Result: " + linearSearch(arr, item1) + ")");

    int item2 = 6;
    System.out.println("Using linearSearch: Is " + item2 + " found? " + (linearSearch(arr, item2) != -1 ? "Yes" : "No") + " (Result: " + linearSearch(arr, item2) + ")");
